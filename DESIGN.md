...
It is **right-aligned** and renders through `UtilityNav.tsx`, a client component — Header is an async server component and no middleware puts the pathname on a request header, so the active tab cannot be resolved server-side. The current tab takes a filled brand ground and a 3px accent rule, drawn as an `inset` box-shadow rather than a border so the tab keeps its full height and nothing shifts between states. Longest path match wins, falling back to the first tab: these are personas rather than pages, so "nothing selected" is never a correct resting state.

**Header actions and the Menu drawer.** The default header carries **no inline nav links on desktop**. The right-hand cluster is `headerActions` (icon + label, e.g. "County", "Log in" — an array of the `OT_HeaderAction` component type, max 4) followed by Search and Menu, which are structural and therefore not modelled in the CMS: neither has a destination an editor could set.

Menu opens `MenuDrawer.tsx`, a panel sliding in from the right holding the primary navigation (items with sub-items expand in place behind a rotating chevron) and a second **Shortcuts** group fed by `menuShortcuts`. Because the drawer is the only route to the primary navigation on a large screen, its dialog semantics are load-bearing rather than decorative: `role="dialog"` + `aria-modal`, a focus trap that wraps at both ends, Escape to close, body scroll lock, and focus returned to the trigger. Both the slide and the overlay fade are `motion-reduce:transition-none`.

Below `lg` the header still renders `MobileMenu`, which owns the small-screen pattern; the drawer is hidden there rather than competing with it. `DesktopNav` is likewise untouched — the split-bar and sidebar header shells still use it.
