...
- Token-driven type system: one **themeable primary family** drives the whole hierarchy (display/headline/title/body/label) via `--ot-font-sans` — Poppins by default, swappable per vertical through the ThemeManager "Primary Font" axis to Source Serif 4, Sora, Plus Jakarta Sans or Manrope (all weight-ladder-matched; see `lib/theme-axes.ts` for the authoritative list). Syne for accent moments only (headline scale and up, at most once per viewport), Geist Mono for code/data, Caveat for the QuoteBlock signature only, Tilt Neon for the PrimaryText "neon" effect only. Reference `--ot-font-sans`, never a raw `--font-poppins`. Serif is allowed only as the Source Serif primary via the axis.
